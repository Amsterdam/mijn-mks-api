...
node {
    ...
    stage('Test') {
        tryStep "test", {
            sh "docker-compose -p mks -f mks/jenkins/test/docker-compose.yml build && " +
               "docker-compose -p mks -f mks/jenkins/test/docker-compose.yml run -u root --rm test"
        }, {
    ...
    stage("Build image") {
        tryStep "build", {
            docker.withRegistry("${DOCKER_REGISTRY}",'docker-registry') {
                def image = docker.build("mijnams/mks:${env.BUILD_NUMBER}", "--build-arg http_proxy=${JENKINS_HTTP_PROXY_STRING} --build-arg https_proxy=${JENKINS_HTTP_PROXY_STRING} --build-arg no_proxy='nexus.secure.amsterdam.nl' .")
                image.push()
            }
        }
    }
